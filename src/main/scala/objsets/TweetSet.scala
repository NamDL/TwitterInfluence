   ...
   */
    def descendingByRetweet: TweetList = {
      if(this.isEmpty) Nil
        ...
        val mostTweeted:Tweet= this.mostRetweeted
...
class Empty extends TweetSet {
    ...
    def mostRetweeted: Tweet= throw new java.util.NoSuchElementException("most tweeted on Empty Set")
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
    ...
    def mostRetweeted: Tweet={
      ...
      else max(left.mostRetweeted,max(elem,left.mostRetweeted))
    }
...
object GoogleVsApple {
  ...
  lazy val googleTweets: TweetSet =   {
    TweetReader.allTweets.filter { 
      tweet => google.exists { 
        lst => tweet.text.contains(lst) 
      } 
    }
  }
    
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter { tweet => apple.exists { lst => tweet.text.contains(lst) } }
...
object Main extends App {
  ...
  //println(size(set5.filter(tw => tw.retweets == 20)))
